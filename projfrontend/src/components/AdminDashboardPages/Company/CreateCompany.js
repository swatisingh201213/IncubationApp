...
import { createcompany, uploadFile } from "../../Auth/CompanyApi";
...
function CreateCompany() {
  ...
  const [eventvalues, setEventValues] = useState(init);

  const url = eventvalues.companylogo
    ? eventvalues.companylogo
  ...
  useEffect(() => {
    const getImage = async () => {
      ...
      if (file) {
        ...
        eventvalues.companylogo = image.data;
        setImage(image.data);
      }
    };
    getImage();
  ...
  const saveCompany = async () => {
    await createcompany(eventvalues);
